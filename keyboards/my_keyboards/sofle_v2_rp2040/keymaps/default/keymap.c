...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [0] = LAYOUT(
        ...
        KC_LGUI,    KC_LALT,    KC_ESC,     KC_SPC,     MO(1),      KC_MUTE,    KC_MUTE,    MO(2),      KC_ENT,     KC_RCTL,    KC_RALT,    KC_RGUI
    ),
    [1] = LAYOUT(
...
const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][NUM_DIRECTIONS] = {
    [0] = {ENCODER_CCW_CW(KC_VOLD, KC_VOLU), ENCODER_CCW_CW(KC_VOLU, KC_VOLD)},
    ...
    if (framenumber != last_bongo_frame) {
        ...
        for (i = 0; i < 4; i++) {
            for (j = 0; j < bongo_line_len[i]; j++) {
                write_bongochar_at_pixel_xy(
                    bongo_line_x[i] + j * 6,
                    bongo_line_y[i],
                    pgm_read_byte(&bongo_line_data[framenumber][current_bongochar]),
                    false);
                current_bongochar++;
            }
        }
